...
> 路径变成 `snapshots/<插件>/<版本>/`。文中指向市场仓库文件的链接已改为绝对地址。
...
所以：**需要最新版时，先按 [`SPEC.md`](./收录规范.md) 真机验证，再新增一套快照，不覆盖旧版。**

> 每条记录的「是否最新版」在 `manifest.json` 的 `isLatest` 字段里。
> 该字段为 `null` 表示**维护者尚未核实**，不是「否」——
> 核实规则见 [`SPEC.md`](./收录规范.md) 第三节第 3 步③，**禁止凭印象填**。
...
详见仓库根目录 [`README.md`](../README.md) 的「30 秒开始」一节，
以及 [`docs/收录说明.md`](./收录快照说明.md)（面向用户的收录说明）。

---

## 如何新增收录

1. 阅读 **[`SPEC.md`](./收录规范.md)** —— 收录规则是**强制**的
