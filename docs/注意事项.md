...
> 本页是[首页](../README.md)「兼容策略」的展开：**事前兼容**而不是事后排错。
> 目标是让用户拿到手就能装对版本，而不是装完启动失败再回来查文档。
>
> ★ **本页提到的几个脚本不在本仓库里**：`scripts/preflight.mjs`、`scripts/dev-env.mjs`、
> `scripts/verify.mjs` 都是**市场仓库**（[`dsh-plugins`](https://github.com/HaydenSmith1121/dsh-plugins)）
> 里的工具 —— 本仓库只负责存放与分发插件，不重复实现一套环境校验。
> 要用它们先 clone 市场仓库，并在**那个仓库的根目录**下运行：
>
> ```bash
> git clone https://github.com/HaydenSmith1121/dsh-plugins
> cd dsh-plugins
> node scripts/dev-env.mjs status      # 先看看隔离环境在不在
> ```
>
> 本仓库自己只有一个脚本：`scripts/build-manifest.mjs`（生成 / 校验 `manifest.json`）。
...
详见[目录结构](https://github.com/HaydenSmith1121/dsh-plugins/blob/main/docs/目录结构.md)与[版本兼容矩阵](./版本兼容矩阵.md)。

### 四道闸门

| 闸门 | 位置 | 挡住什么 |
|---|---|---|
| 环境预检 | 市场仓库 `scripts/preflight.mjs`（只读） | Node / pnpm / dsh 版本不对、pnpm 装错 Node |
| 装前兼容性检查 | 插件市场面板 | 环境 / profile / 候选包三层，致命项硬拦截 |
| 四步校验 | 市场仓库 `scripts/verify.mjs` | 版本 / 依赖层 / 装配层 / 真实启动 |
| 收录快照 | 本仓库 `snapshots/` + `manifest.json` | 上游更新后装不回当初验证过的那一版 |
...
```bash
# 1) 把 profile 里的 file: 路径改成新位置（或直接重新 add 一遍）
...
```bash
# 以下命令都在**市场仓库**根目录跑；tarball 路径用本仓库里的那一份
dsh plugin --profile web remove <包名>
node scripts/dev-env.mjs install <本仓库>/plugins/<id>/<dsh版本>/<包名>-<版本>.tgz
```
...
[`README-开发环境隔离.md`](https://github.com/HaydenSmith1121/dsh-plugins/blob/main/README-开发环境隔离.md) 里的隔离脚手架
...
详见[版本兼容矩阵 § 两条必须记住的规矩](./版本兼容矩阵.md#二两条必须记住的规矩)。
...
**市场面板起不来时**，用「体检」页的**快照回滚**，或者直接跑一次校验看是哪一层出的问题：
...
完整来历与改动清单见
[`README-安装说明.md` 第十节](https://github.com/HaydenSmith1121/dsh-plugins/blob/main/README-安装说明.md#十关于-dsh-opencode-go-plus-的来历改造与共存禁忌)。

---

**[← 回到首页](../README.md)** ·
[安装教程](https://github.com/HaydenSmith1121/dsh-plugins/blob/main/README-安装说明.md) ·
[目录结构](https://github.com/HaydenSmith1121/dsh-plugins/blob/main/docs/目录结构.md) ·
[版本兼容矩阵](./版本兼容矩阵.md) ·
[收录说明](./收录快照说明.md) ·
[插件清单与来源](https://github.com/HaydenSmith1121/dsh-plugins/blob/main/docs/插件清单与来源.md)
