...
> ## ⚠️ 本仓库**停止维护**，将被丢弃
>
> 自研插件正在**逐个迁到各自的源码仓库**，安装统一走**插件市场**面板
> （`dsh-plugins-market`）—— 本仓库不再接收新版本。已迁走的例子：
> [`dsh-usage-stats`](https://github.com/HaydenSmith1121/dsh-usage-stats)、
> [`dsh-chat-fold`](https://github.com/HaydenSmith1121/dsh-chat-fold)、
> [`dsh-connect-trae-plus`](https://github.com/HaydenSmith1121/dsh-connect-trae-plus)。
>
> 下面这些 tarball 与快照**原样保留**（已发布的字节不改写），但**不会再有新版本**；
> 需要最新版请去各插件的源码仓库，或直接装市场里那一条。

**共 7 个插件 / 9 个版本**（本仓库最后收录的状态），全部以 dsh `0.1.6-alpha.1` 为基线。
...
| [`dsh-workbuddy-quota`](#dsh-workbuddy-quota) | `0.2.0` ⚠️ 不再维护 | WorkBuddy 剩余额度 pill（用量统计部分已由 [`dsh-usage-stats`](https://github.com/HaydenSmith1121/dsh-usage-stats) 接手） | tarball → `dsh plugin add` |
...
### 7. `dsh-workbuddy-quota` — WorkBuddy 额度与用量（⚠️ 不再维护）

> **用量统计这一半已被 [`dsh-usage-stats`](https://github.com/HaydenSmith1121/dsh-usage-stats) 接手**
> （它已迁到自己的源码仓库，不在本仓库收录）。0.2.0 的用量统计现场折叠会话日志，
> 而日志会被 `dsh-session-cleanup` 删掉 —— **删一次会话就丢一次用量**；`dsh-usage-stats@0.3.1`
> 用只增不减的台账修掉了它。本版保留的是**回滚用的原样字节**，以及只有本包才有的 **WorkBuddy 额度 pill**。
