...
> ## ⚠️ 已被 [`dsh-usage-stats`](https://github.com/HaydenSmith1121/dsh-usage-stats) 取代
>
...
>
> `dsh-usage-stats@0.3.1` 在折叠之外加了一份**只增不减的每会话用量台账**，
> 从结构上修掉了它，并**只保留用量统计**这一半。它已迁到自己的源码仓库
> （本仓库不再收录它）。本页保留的是**回滚用的原样字节**，
> 以及只有本包才有的 **WorkBuddy 额度 pill** 的说明。
>
> | 你想要 | 装哪个 |
> |---|---|
> | Token 用量统计（且删会话不丢账） | [`dsh-usage-stats`](https://github.com/HaydenSmith1121/dsh-usage-stats) `0.3.1`（装法见该仓库 README，或直接装市场里那一条） |
